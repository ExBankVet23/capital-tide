...
FREE SOURCES USED (all confirmed free-tier / keyless as of build time):
  ...
  - Binance public futures API       — no key (market data endpoints only).
                                        Falls back to Bybit's public API if
                                        Binance returns a 451 (a real, confirmed
                                        issue: Binance geo-blocks GitHub Actions'
                                        US-based runner IPs — works fine locally,
                                        fails specifically when run via GitHub)
...
# ---------------------------------------------------------------------------
# Perp Funding Rate + Open Interest — via CoinGecko, not exchanges directly
#
# A real GitHub Actions run confirmed BOTH Binance (451 "Unavailable For
# Legal Reasons") and Bybit (403, confirmed via independent research to be a
# CDN-level cloud-IP block — GitHub-hosted runners share IP space with
# Azure, and major exchanges blanket-block AWS/Azure ranges as an anti-abuse
# measure) refuse GitHub Actions' runner IPs. This isn't fixable by trying a
# third exchange directly — they very likely apply the same blocking policy.
#
# The fix: stop hitting exchanges directly. CoinGecko's free /derivatives
# endpoint proxies this same data without touching an exchange's own
# CDN-protected infrastructure, and CoinGecko has been completely reliable
# from this exact GitHub Action throughout the rest of this pipeline.
#
# One real limitation: CoinGecko's free tier only gives a CURRENT snapshot
# here, not history (same constraint we hit with BTC Dominance). So this
# accumulates its own history — one new data point saved every day the
# Action runs, read back in and appended to on the next run. Thin at first,
# genuinely useful within a few weeks. The workflow file commits
# derivatives_history.json alongside data.json so this state persists.
# ---------------------------------------------------------------------------
DERIVATIVES_HISTORY_FILE = "derivatives_history.json"


def coingecko_derivatives_snapshot(exchange_id="binance_futures", symbol_match="BTCUSDT"):
    url = f"https://api.coingecko.com/api/v3/derivatives/exchanges/{exchange_id}"
    ...
    match = next((t for t in tickers if str(t.get("symbol", "")).upper().replace("-", "").replace("/", "") == symbol_match), None)
    if not match:
        raise RuntimeError(f"could not find a {symbol_match} ticker on {exchange_id} via CoinGecko derivatives endpoint")
    return match


def load_derivatives_history():
    if os.path.exists(DERIVATIVES_HISTORY_FILE):
        try:
            with open(DERIVATIVES_HISTORY_FILE) as f:
                return json.load(f)
        except Exception:
            pass
    return {"funding_rate": [], "open_interest_b": [], "last_date": None}


def save_derivatives_history(hist):
    with open(DERIVATIVES_HISTORY_FILE, "w") as f:
        json.dump(hist, f)


DERIVATIVES_HISTORY_MAX = 500  # matches the keep_last=500 pattern used elsewhere in this pipeline


def accumulate_derivatives_history():
    hist = load_derivatives_history()
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    if hist.get("last_date") != today:
        snap = coingecko_derivatives_snapshot("binance_futures", "BTCUSDT")
        fr = snap.get("funding_rate")
        oi = snap.get("open_interest_usd")
        if fr is not None:
            hist["funding_rate"].append(float(fr))
        if oi is not None:
            hist["open_interest_b"].append(float(oi) / 1e9)
        # Cap both lists so this file doesn't grow forever — keep only the
        # most recent DERIVATIVES_HISTORY_MAX days, same trimming pattern
        # every other series in this pipeline already uses.
        hist["funding_rate"] = hist["funding_rate"][-DERIVATIVES_HISTORY_MAX:]
        hist["open_interest_b"] = hist["open_interest_b"][-DERIVATIVES_HISTORY_MAX:]
        hist["last_date"] = today
        save_derivatives_history(hist)
    else:
        print("  (derivatives_history.json already has today's snapshot — not re-fetching)")
    return hist


def funding_rate_with_fallback(symbol="BTCUSDT"):
    hist = accumulate_derivatives_history()
    vals = hist.get("funding_rate", [])
    if len(vals) < 10:
        raise RuntimeError(f"only {len(vals)} days accumulated so far (via CoinGecko snapshots) — "
                            f"needs a few more days of the Action running before this has enough "
                            f"history to use; not a failure, just early days")
    return vals


def open_interest_with_fallback(symbol="BTCUSDT"):
    hist = accumulate_derivatives_history()
    vals = hist.get("open_interest_b", [])
    if len(vals) < 5:
        raise RuntimeError(f"only {len(vals)} days accumulated so far (via CoinGecko snapshots) — "
                            f"needs a few more days of the Action running before this has enough "
                            f"history to use; not a failure, just early days")
...
safe("Perp Funding Rate (BTC)", CRYPTO_EXTRA, funding_rate_with_fallback, "BTCUSDT")
safe("Open Interest ($B)", CRYPTO_EXTRA, open_interest_with_fallback, "BTCUSDT")
