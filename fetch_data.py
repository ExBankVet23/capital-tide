...
# ---------------------------------------------------------------------------
# Bybit public API — fallback for the two Binance endpoints above.
# A real GitHub Actions run confirmed Binance returns 451 "Unavailable For
# Legal Reasons" from GitHub's US-based runner IPs — a regulatory geo-block,
# not a code issue. This worked fine locally (different IP) but fails from
# GitHub's servers specifically. Bybit's public market-data endpoints (no
# key/account needed) are commonly used as a fallback in exactly this
# situation since they don't apply the same cloud-IP blocking.
# ---------------------------------------------------------------------------
def bybit_funding_rate_history(symbol="BTCUSDT", limit=200):
    url = "https://api.bybit.com/v5/market/funding/history"
    r = SESSION.get(url, params={"category": "linear", "symbol": symbol, "limit": limit}, timeout=20)
    r.raise_for_status()
    data = r.json()
    rows = (data.get("result") or {}).get("list") or []
    if not rows:
        raise RuntimeError(f"Bybit returned no rows (retMsg: {data.get('retMsg')})")
    # Bybit returns newest-first — reverse to chronological, matching every other series here.
    vals = [float(x["fundingRate"]) for x in reversed(rows)]
    if len(vals) < 10:
        raise RuntimeError("insufficient data returned")
    return vals


def bybit_open_interest_hist(symbol="BTCUSDT", interval="1d", limit=30):
    url = "https://api.bybit.com/v5/market/open-interest"
    r = SESSION.get(url, params={"category": "linear", "symbol": symbol, "intervalTime": interval, "limit": limit}, timeout=20)
    r.raise_for_status()
    data = r.json()
    rows = (data.get("result") or {}).get("list") or []
    if not rows:
        raise RuntimeError(f"Bybit returned no rows (retMsg: {data.get('retMsg')})")
    vals = [float(x["openInterest"]) / 1e9 for x in reversed(rows)]  # roughly comparable $B scale to the Binance version
    if len(vals) < 5:
        raise RuntimeError("insufficient data returned")
    return vals
...
def funding_rate_with_fallback(symbol="BTCUSDT"):
    try:
        return binance_funding_rate_history(symbol)
    except Exception as e_binance:
        try:
            return bybit_funding_rate_history(symbol)
        except Exception as e_bybit:
            raise RuntimeError(f"Binance failed ({e_binance}); Bybit fallback also failed ({e_bybit})")


def open_interest_with_fallback(symbol="BTCUSDT"):
    try:
        return binance_open_interest_hist(symbol)
    except Exception as e_binance:
        try:
            return bybit_open_interest_hist(symbol)
        except Exception as e_bybit:
            raise RuntimeError(f"Binance failed ({e_binance}); Bybit fallback also failed ({e_bybit})")
